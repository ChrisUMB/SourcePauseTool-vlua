...
void LuaFeature::LoadFeature() {
    ...
    if (JumpSignal.Works) {
        void (*jump)() = []() {
            lua_events_library.InvokeEvent("player_jump", [](lua_State* L) {
                lua_newtable(L);
            });
        };
    ...
    if (DemoStartPlaybackSignal.Works) {
        void (*demo_start_playback)(const char*, bool as_time_demo) =
            ...
            lua_events_library.InvokeEvent("demo_start", [&](lua_State* L) {
                lua_newtable(L);
...
int LuaFeature::HOOKED_NET_RunFrame(double time) {
    ...
    if (IsGameSimulating()) {
        lua_events_library.InvokeEvent("sim_tick", [](lua_State* L) {
            lua_newtable(L);
        });
    }
